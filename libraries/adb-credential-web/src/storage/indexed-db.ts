...
async function openDatabase() {
  ...
  if (db.version === 1) {
    const keys = await createTransaction(db, (tx) =>
      waitRequest(tx.objectStore(StoreName).getAll() as IDBRequest<Uint8Array[]>)
    );
...
function createTransaction<T>(database: IDBDatabase, callback: (transaction: IDBTransaction) => T): Promise<T> {
  return new Promise<T>((resolve, reject) => {

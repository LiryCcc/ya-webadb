...
export class TangoPasswordProtectedStorage implements TangoKeyStorage {
  ...
  constructor(storage: TangoKeyStorage, requestPassword: TangoPasswordProtectedStorage.RequestPassword) {
    this.#storage = storage;
    this.#requestPassword = requestPassword;
  }

  async save(privateKey: Uint8Array<ArrayBuffer>, name: string | undefined): Promise<undefined> {
  ...
  async *load(): AsyncGenerator<MaybeError<TangoKey>, void, void> {
    for await (const result of this.#storage.load()) {
      ...
      try {
        ...
        const { aesKey } = await deriveAesKey(password, bundle.pbkdf2Salt as Uint8Array<ArrayBuffer>);

...
function checkCredential(credential: Credential | null): asserts credential is PublicKeyCredential {
...
export class TangoWebAuthnPrfSource implements TangoPrfSource {
   ...
   * @throws `OperationCancelledError` if the attestation is either cancelled by user or timed out
   */
  async get(id: BufferSource, input: Uint8Array<ArrayBuffer>): Promise<BufferSource> {
    const challenge = new Uint8Array(32);
    crypto.getRandomValues(challenge);

...
export type AdbShellProtocolSpawner = (
  ...
  AdbSubprocessSpawner.Wait<AdbShellProtocolSpawner.WaitResult<Uint8Array>, AdbShellProtocolSpawner.WaitResult<string>>;
...
export function adbShellProtocolSpawner(
  spawn: (command: readonly string[], signal: AbortSignal | undefined) => Promise<AdbShellProtocolProcess>
  ...
  return (command, signal) => {
    ...
    const processPromise = spawn(command, signal) as Promise<AdbShellProtocolProcess> &

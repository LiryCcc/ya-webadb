...
export type AdbNoneProtocolSpawner = (
  ...
  signal?: AbortSignal
) => Promise<AdbNoneProtocolProcess> & AdbSubprocessSpawner.Wait<Uint8Array, string>;

export function adbNoneProtocolSpawner(
  spawn: (command: readonly string[], signal: AbortSignal | undefined) => Promise<AdbNoneProtocolProcess>
  ...
  return (command, signal) => {
    ...
    const processPromise = spawn(command, signal) as Promise<AdbNoneProtocolProcess> &

...
export type AdbDaemonAuthenticateOptions = Pick<
  ...
  'serial' | 'connection' | 'features' | 'preserveConnection' | 'readTimeLimit'
> & {
  /**
...
export async function adbDaemonAuthenticate({
  ...
  const processor =
    'processor' in processorInit ? processorInit.processor : new AdbDaemonDefaultAuthProcessor(processorInit);

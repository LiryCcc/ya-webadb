...
export class PauseControllerImpl implements ScrcpyVideoDecoderPauseController {
  ...
  #onConfiguration: (packet: ScrcpyMediaStreamConfigurationPacket) => MaybePromiseLike<undefined>;
  #onFrame: (packet: ScrcpyMediaStreamDataPacket, skipRendering: boolean) => MaybePromiseLike<undefined>;
  ...
  async #resumeInternal(explicitly: boolean): Promise<undefined> {
    ...
    for (let i = 0, length = this.#pendingFrames.length; i < length; i += 1) {

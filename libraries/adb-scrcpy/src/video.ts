...
import type { ScrcpyMediaStreamPacket, ScrcpyVideoSize, ScrcpyVideoStreamMetadata } from '@yume-chan/scrcpy';
...
export class AdbScrcpyVideoStream implements ScrcpyVideoSize {
  ...
  constructor(
    ...
    this.#stream = stream.pipeThrough(this.#options.createMediaStreamTransformer()).pipeThrough(
      new InspectStream(
        (packet): undefined => {
          ...
          } else if (this.#metadata.codec === ScrcpyVideoCodecId.AV1) {

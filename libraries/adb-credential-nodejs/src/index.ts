  ...
  async *#readVendorKeys(path: string): AsyncGenerator<MaybeError<TangoKey>, void, void> {
    ...
    if (stats.isFile()) {
      try {
        ...
        yield e instanceof KeyError ? e : new VendorKeyError(path, { cause: e });
        return;
      }
    }
    ...
    if (stats.isDirectory()) {
      ...
      for await (const dirent of dir) {
        ...
        try {
          ...
          yield e instanceof KeyError ? e : new VendorKeyError(file, { cause: e });
        }
      }
    }
  }
  ...
  async *load(): AsyncGenerator<MaybeError<TangoKey>, void, void> {
    ...
    if (existsSync(userKeyPath)) {
      try {
        ...
        yield e instanceof KeyError ? e : new InvalidKeyError(userKeyPath, { cause: e });
      }
    }

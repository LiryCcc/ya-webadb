...
export function defaultFieldSerializer<T>(serializer: DefaultFieldSerializer<T>): FieldSerializer<T>['serialize'] {
  return (source, context: FieldDefaultSerializeContext | FieldByobSerializeContext): never => {
...
export type ByobFieldSerializer<T> = (source: T, context: FieldByobSerializeContext & { index: number }) => void;

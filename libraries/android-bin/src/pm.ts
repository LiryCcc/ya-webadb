...
export type PackageManagerInstallOptions = {
  [K in keyof typeof PackageManagerInstallOptions]?: (typeof PackageManagerInstallOptions)[K]['type'] | undefined;
};
...
function buildInstallCommand(
  ...
  for (const [key, value] of Object.entries(options)) {
    ...
    const option = PackageManagerInstallOptions[key as keyof PackageManagerInstallOptions];
...
export class PackageManager extends AdbServiceBase {
   ...
   */
  async install(apks: readonly string[], options?: PackageManagerInstallOptions): Promise<void> {
  ...
  async *listPackages(
    ...
    const command = buildCommand(['package', 'list', 'packages'], options, PACKAGE_MANAGER_LIST_PACKAGES_OPTIONS_MAP);
  ...
  async uninstall(packageName: string, options?: Optional<PackageManagerUninstallOptions>): Promise<void> {
   ...
   */
  async sessionCreate(options?: PackageManagerInstallOptions): Promise<number> {

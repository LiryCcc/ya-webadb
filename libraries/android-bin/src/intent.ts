...
function serializeExtraArray(array: IntentNumberArrayExtra | IntentStringArrayExtra): [type: string, value: string] {
  let type: string;
  let value: string;

  if (array.itemType === 'string') {
    type = '--es';
    value = array.value.map((item) => item.replaceAll(',', '\\,')).join(',');
  } else {
  ...
  if (intent.extras) {
    for (const [key, value] of Object.entries(intent.extras)) {
      switch (typeof value) {
        ...
        case 'number':
          if (value % 1 !== 0) {
            throw new Error(`Extra \`${key}\` is not an integer, must use \`IntentNumberExtra\` type instead`);
          }
